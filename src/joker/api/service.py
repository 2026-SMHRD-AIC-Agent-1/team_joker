...
def make_worker(prep: dict, repo, on_progress=None, should_cancel=None, detector=None):
    """진단 실행 + 저장을 하는 무인자 함수. 여기서만 SQLite 에 쓴다(잡 워커 스레드).

    on_progress:  엔진이 단계·실행 수를 알려주는 콜백(선택). 폴링 응답의 진행 표시에 쓴다.
    should_cancel: 취소 요청을 확인하는 콜백(선택). True 면 엔진이 DiagnosisCancelled 를
      던지고 **아래 repo.save_run 에 도달하지 못한다** — 반쪽짜리 진단이 이력에 남지 않는다.
    """
    ...
    def _work() -> None:
        deps = Deps(
            ...
            on_progress=on_progress, should_cancel=should_cancel, detector=detector,
        )
