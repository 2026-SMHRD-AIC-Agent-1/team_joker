...
HONESTY = [
    ...
    "연결된 문서·외부 기능·이전 대화는 포함하지 않습니다",         # SCOPE_NOTICE
    "저장하지 않으므로 단정하지 않습니다",                          # 해결됨 권고
    "수정 후 남은 공격도 추가로 검사합니다",                           # 관계도
    ...
    "실제 상세 내용은 가입 후 불러옵니다",                                       # 게이트 — 서버가 안 보낸다
    "검사할 공격 수는 검사 중 달라질 수 있습니다",                                # 진행 화면
    "키는 저장하지 않습니다",                                       # BYOK
    "선택한 AI 모델에 연결할 수 없어",                               # 대상 모델 연결 실패
    "규칙 검사 결과만 제공합니다",
    "이 수치는 탐지 성능 지표가 아닙니다",                          # 사후 검사 — 학습 시드와 겹침(순환 평가)                                                    # 탐지기 근거 수치의 하한 단서
]
