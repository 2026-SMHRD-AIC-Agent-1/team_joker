  ...
  return (
    <div className="tbl-wrap">
      <div className="table-scroll"><table className="tbl" aria-label={label}>
        <thead>
          <tr><th>상태</th><th>진단 번호 · 날짜</th><th>등급</th><th>유출 비율 · 수정 전 → 후</th><th>진단 대상 모델</th><th /></tr>
        </thead>
        <tbody>
          {runs.map((r) => {
              ...
              <tr key={r.run_id}>
                <td>{r.status === "running" ? <span className="pill">진행 중</span> : r.status === "error" ? <span className="pill">실패</span> : r.inconclusive ? <span className="pill">진단 불가</span> : n ? (
                  <span className="badge" style={{ color: sev("unresolved") }}>
                    <i style={{ background: sev("unresolved") }} />확인 필요 {n}건
                  </span>
                ...
                <td><span className="cell-sub">{r.target_model ?? "-"}</span>
                  {isMock(r) ? <span className="tag-mock">예시 결과</span> : null}</td>
                <td className="act"><Link to={`/runs/${encodeURIComponent(r.run_id)}`}>결과 보기 →</Link></td>
              </tr>
            );
          })}
        </tbody>
