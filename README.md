...
화면에 기능이 두 개(**진단**, **입력 탐지**) 보이는데, 별개가 아니라 **한 챗봇을 지키는 두 겹**입니다.

```mermaid
flowchart LR
    U["사용자 입력"] --> L1{"① JOKER-KO 탐지기<br/>런타임 · 매 요청 · 수십 ms"}
    L1 -->|"공격이면 여기서 차단"| X["챗봇 호출 안 함"]
    L1 -->|정상| L2["② 보강한 지시문 + 모델<br/>배포 전 감사가 만든 결과물"]
    L2 --> R["응답"]
```

| | ① JOKER-KO 탐지기 | ② 진단 엔진 |
|---|---|---|
| 무엇을 보나 | **들어오는 입력** — 이 문장이 공격인가 | **나가는 응답** — 비밀이 샜는가 |
| 언제 도나 | 운영 중, 요청마다 | 배포 전, 지시문당 1회 |
| 속도 | 수십 ms | 30초 ~ 수 분 |
| 결과 | SAFE / INJECTION | 보강안 + 전후 유출 건수 |

### 두 층이 서로를 필요로 하는 이유

**②만으로는 부족합니다.** 지시문을 아무리 잘 고쳐도 모델이 넘어가는 공격이 남습니다 — 실측으로 **14%** 가 남았습니다.
**①만으로도 부족합니다.** 탐지기를 통과한 요청은 결국 모델이 처리하므로, 지시문 자체가 약하면 그대로 뚫립니다.

**둘을 같이 쓰면 잔여 유출이 0건 / 50** 이었습니다([5-③](#metric-defense-matrix)).

> **②의 진단 결과가 ①의 배치를 처방합니다.** 진단이 "보강 후에도 이런 계열이 남았다"고 알려주면,
> 그게 곧 입력단에 탐지기를 붙여야 하는 이유와 그 근거 건수가 됩니다.
> 화면의 `권고 조치` 에 그 숫자가 그대로 나옵니다.
...
flowchart TD
    ...
    F --> D
    F --> G["입력 탐지 테스트<br/>남은 공격 문구를 ①에 넣어보기"]
```
...
`입력 탐지 테스트` 화면의 예시 버튼 중 '난독화' 로 그 장면을 직접 만들어 볼 수 있습니다.
...
| 화면 6개 (인증 · 새 진단 · 결과 · 대시보드 · 진단 목록 · 입력 탐지 · 설정) | 완료, UI/UX 다듬는 중 |
| 자동화 테스트 | 458개 |

> 학습된 탐지 모델(`detector/artifacts/joker-ko`)은 1.1GB 라 저장소에 커밋하지 않습니다.
> 그 폴더가 없는 PC 에서는 **진단은 정상 동작하고, 입력 탐지 화면만 안내가 뜹니다.**
